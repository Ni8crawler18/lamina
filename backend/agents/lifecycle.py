...
from backend.hedera.token import create_token, mint_tokens, burn_tokens, transfer_tokens
from backend.hedera.consensus import create_topic, log_agent_action
...
async def distribute_coupon(asset_id: int) -> dict:
    ...
    from backend.hedera.token import transfer_hbar
...
async def update_nav_from_oracle(asset_id: int) -> dict:
    ...
    from backend.oracle.treasury_rates import get_yield_for_duration
...
async def execute_maturity(asset_id: int) -> dict:
    ...
    from backend.hedera.token import wipe_tokens, transfer_hbar
